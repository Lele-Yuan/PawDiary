...
Component({
  ...
  methods: {
    ...
    /**
     * 兼容多种错误结构，提取出可读的 message 字符串与业务 code。
     * 已知形态：
     *   1) { code, message: 'xxx' }
     *   2) { message: { name: 'AI_RetryError', errors: [{ responseBody: '{"code":"EXCEED_TOKEN_QUOTA_LIMIT","message":"..."}', ... }] } }
     *   3) { message: { reason, errors: [...] } }
     *   4) message 直接是普通对象 / 字符串
     */
    _extractErrorInfo(error) {
      const result = { code: error && error.code, message: "" };
      ...
      // 嵌套对象（如 AI_RetryError）
      if (msg && typeof msg === "object") {
        // 优先尝试解析 errors[].responseBody 里的 JSON
        const errors = Array.isArray(msg.errors) ? msg.errors : [];
        for (let i = errors.length - 1; i >= 0; i--) {
          const inner = errors[i] || {};
          if (inner.responseBody && typeof inner.responseBody === "string") {
            try {
          ...
          if (!result.message && inner.message && typeof inner.message === "string") {
            result.message = inner.message;
          }
        }
        // 兜底用 reason 或 name
        if (!result.message) {
          result.message = msg.reason || msg.name || "";
        }
      }
      // 实在没拿到，避免 [object Object]
      if (!result.message || typeof result.message !== "string") {
        result.message = "网络繁忙，请稍后重试！";
      }
      // 已知 code 友好化
      if (result.code === "EXCEED_TOKEN_QUOTA_LIMIT") {
        result.message = "大模型 Token 已耗尽，请通知开发者前往云开发平台进行处理";
      }
      return result;
    },
          ...
          try {
            ...
            if (finish_reason === "error" || finish_reason === "content_filter" || error) {
              ...
              if (error && error.message) {
                // 兼容 error.message 为对象 / 嵌套 AI_RetryError 的情况
                const parsed = this._extractErrorInfo(error);
                if (parsed.code === "EXCEED_TOKEN_QUOTA_LIMIT") {
                  lastValue.content = "大模型 Token 已耗尽，请通知开发者前往云开发平台进行处理";
                  lastValue.error = lastValue.content;
                } else if (parsed.message) {
                  lastValue.content = parsed.message;
                  lastValue.error = parsed.message;
                } else {
                  lastValue.content = this.data.defaultErrorMsg;
                  lastValue.error = this.data.defaultErrorMsg;
                }
                this.setData({
